...
import type { ExpenseRecord } from '@/lib/schemas';
...
export default function useRecorder(): UseRecorderReturn {
  ...
  const [extracted, setExtracted] = useState<ExpenseRecord | null>(null);
  ...
  const uploadAudio = useCallback(async (): Promise<UploadResult | null> => {
    ...
    try {
      ...
      if (!payload) {
        ...
        setStatus('error');
        setExtracted(null);
        return { transcript: null, extracted: null, mode: 'audio', error: 'Réponse serveur invalide' };
      }

      const transcript = typeof payload.transcript === 'string' ? payload.transcript : null;
      const extractedExpense = payload.extracted ? (payload.extracted as ExpenseRecord) : null;
      const payloadMode: SubmissionMode = payload.mode === 'text' ? 'text' : 'audio';
      ...
      if (!response.ok) {
        ...
        return { transcript, extracted: extractedExpense, mode: 'audio', error: message };
      }

      setTranscription(transcript);
      setExtracted(extractedExpense);
      setLastMode(payloadMode);
      setAudioBlob(null);
      setStatus('success');
      ...
      return { transcript, extracted: extractedExpense, mode: payloadMode };
    } catch (err) {
    ...
    async (text: string): Promise<UploadResult | null> => {
      ...
      try {
        ...
        if (!payload) {
          ...
          setStatus('error');
          setExtracted(null);
          return { transcript: null, extracted: null, mode: 'text', error: 'Réponse serveur invalide' };
        }
        ...
        if (!response.ok) {
          ...
          setTranscription(transcript);
          setLastMode(payloadMode);
          return { transcript, extracted: extractedExpense, mode: payloadMode, error: message };
        }
